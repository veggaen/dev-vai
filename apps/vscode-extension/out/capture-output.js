...
exports.registerOutputWatchers = registerOutputWatchers;
...
function processDiagnostics(uris) {
    ...
    (0, session_js_1.pushEvent)('state-change', content, {
        state: 'diagnostics',
        errors: totalErrors,
...
/* ── Register Watchers ─────────────────────────────────────────── */
function registerOutputWatchers(context) {
    const cfg = vscode.workspace.getConfiguration('vai');
    if (!cfg.get('captureOutputChannels', true))
        return;
    ...
    context.subscriptions.push(vscode.tasks.onDidStartTask((e) => {
        ...
        (0, session_js_1.pushEvent)('terminal', `Task started: ${task.name}`, {
            command: task.name,
    ...
    context.subscriptions.push(vscode.tasks.onDidEndTask((e) => {
        if (!(0, session_js_1.getActiveSession)())
            return;
        const task = e.execution.task;
        (0, session_js_1.pushEvent)('note', `Task ended: ${task.name}`, {
            taskName: task.name,
        });
    }));
    ...
    context.subscriptions.push(vscode.debug.onDidStartDebugSession((session) => {
        if (!(0, session_js_1.getActiveSession)())
            return;
        (0, session_js_1.pushEvent)('state-change', `Debug started: ${session.name}`, {
            state: 'debugging',
    ...
    context.subscriptions.push(vscode.debug.onDidTerminateDebugSession((session) => {
        if (!(0, session_js_1.getActiveSession)())
            return;
        (0, session_js_1.pushEvent)('note', `Debug ended: ${session.name}`, {
            sessionName: session.name,
        });
    }));
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((e) => {
        ...
        if (e.affectsConfiguration('github.copilot') || e.affectsConfiguration('vai')) {
            (0, session_js_1.pushEvent)('note', 'Configuration changed', {
