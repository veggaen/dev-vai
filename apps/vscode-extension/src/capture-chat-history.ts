...
let disposed = false;
let lastSeenVSCodeSessionId: string | null = null; // Track which VS Code chat we're in
let lastSeenVSCodeTitle: string | null = null; // Track the VS Code chat title for sync
...
const vscodeToDevLogs = new Map<string, string>();
...
// Buffer response text per-request so streaming fragments get combined into ONE message.
// VS Code sends many incremental response patches during streaming. Without buffering,
// a single assistant turn can generate 10-20 fragment messages.
...
// are queued here and replayed when the session becomes available.
interface QueuedEvent {
  handler: () => void;
  description: string;
}
const preSessionQueue: QueuedEvent[] = [];
 ...
 */
export function registerChatHistoryWatcher(context: vscode.ExtensionContext): void {
...
function watchForDirectoryCreation(parentDir: string, context: vscode.ExtensionContext): void {
  try {
...
function startWatching(context: vscode.ExtensionContext): void {
  if (!chatSessionsDir || disposed) return;
  ...
  if (!sessionListenerRegistered) {
    ...
    const disposable = onSessionChange((session) => {
      if (!session) return;
      ...
      if (lastSeenVSCodeTitle && lastSeenVSCodeTitle !== session.title) {
        void (async () => {
          try {
            await apiCall(`/api/sessions/${session.id}`, 'PATCH', {
  ...
  try {
    fileWatcher = fs.watch(chatSessionsDir, (eventType, filename) => {
      if (!filename || !filename.endsWith('.jsonl') || disposed) return;
...
function startPolling(): void {
  ...
  pollInterval = setInterval(() => {
    if (disposed || !chatSessionsDir) return;
    pollForChanges();
  }, POLL_INTERVAL_MS);
}
...
async function readNewContent(filename: string): Promise<void> {
  if (!chatSessionsDir || disposed) return;
 ...
 */
async function ensureCorrectDevLogsSession(vscodeSessionId: string): Promise<void> {
  const mappedId = vscodeToDevLogs.get(vscodeSessionId);
  if (!mappedId) return; // Not mapped yet — will be handled when header is processed

  const current = getActiveSession();
  if (current?.id === mappedId) return; // Already correct

  console.log(`[vai:chat-history] 🔄 Switching dev logs session to ${mappedId} for VS Code chat ${vscodeSessionId}`);
  const ok = await attachToSession(mappedId, `VS Code Chat`);
  if (!ok) {
 ...
 */
function writeVaiSessionFile(sessionId: string, title: string): void {
 ...
 */
async function createNewChatSession(title: string, vscodeSessionId: string): Promise<string | null> {
  try {
    ...
    pushEvent('state-change', `New chat: ${title}`, {
      state: 'New chat',
      detail: title,
      vscodeSessionId,
 ...
 */
function bufferResponseText(requestIndex: number, combinedText: string): void {
  ...
  if (existing && combinedText.length <= existing.combinedText.length) return;
...
async function processJsonlLine(obj: unknown, filename?: string): Promise<void> {
  ...
  if (record.kind === 0 && record.v && typeof record.v === 'object') {
    const header = record.v as Record<string, unknown>;
    const customTitle = header.customTitle as string | undefined;
    const vscodeSessionId = header.sessionId as string | undefined;

    if (!vscodeSessionId) return;
    ...
    if (mappedId) {
      ...
      if (current?.id !== mappedId) {
        ...
        const ok = await attachToSession(mappedId, title);
        if (!ok) {
    ...
    if (existing && sessionAge < 120_000 && !vscodeToDevLogs.has(vscodeSessionId)) {
      ...
      try {
        const patchBody: Record<string, string> = { title };
        ...
        await apiCall(`/api/sessions/${existing.id}`, 'PATCH', patchBody);
      ...
      pushEvent('state-change', `Chat: ${title}`, {
  ...
  if (!Array.isArray((record as Record<string, unknown>).k)) return;
  const keyPath = ((record as Record<string, unknown>).k as unknown[]).map((x: unknown) => typeof x === 'number' ? 'N' : x).join('.');
  ...
  if (record.kind === 2 && keyPath === 'requests' && Array.isArray(record.v)) {
    const requests = record.v as Record<string, unknown>[];
    for (let i = 0; i < requests.length; i++) {
      const req = requests[i];
      if (!req || typeof req !== 'object') continue;

      // User message: req.message.text
      const reqMsg = (req as Record<string, unknown>).message as Record<string, unknown> | undefined;
      if (reqMsg?.text && typeof reqMsg.text === 'string' && (reqMsg.text as string).length > 0) {
        handleUserMessage(reqMsg.text as string);
      }

      // If this request already has inline response data, process it using buffered approach
      if (Array.isArray((req as Record<string, unknown>).response)) {
        const responseItems = (req as Record<string, unknown>).response as any[];
        // Process thinking and tool invocations immediately
        for (const item of responseItems) {
          if (!item || typeof item !== 'object') continue;
          if (item.kind === 'thinking' && item.value && item.value.length > 0) {
            handleThinkingBlock(item as ThinkingBlock);
          }
        ...
        const textItems = responseItems.filter((item: any) =>
          item && typeof item === 'object' && !('kind' in item) &&
          item.value && typeof item.value === 'string' && item.value.length > 30
        );
        if (textItems.length > 0) {
          const combinedText = textItems.map((item: any) => item.value).join('\n\n');
  ...
  if (record.kind === 2 && keyPath === 'requests.N.response' && Array.isArray(record.v)) {
    const kArray = (record as Record<string, unknown>).k as unknown[];
    const requestIndex = typeof kArray[1] === 'number' ? kArray[1] : 0;
    const items = record.v as any[];
...
function handleThinkingBlock(block: ThinkingBlock): void {
  ...
  pushEvent('thinking', block.value, {
    source: 'chat-history-watcher',
  ...
  if (block.value.length > 100) {
    ...
    pushEvent('state-change', `Thinking...\n${statusLine}`, {
      state: 'Thinking...',
      detail: statusLine,
      source: 'chat-history-watcher',
      autoCapture: true,
 ...
 */
function detectPlanInResponse(text: string): void {
  ...
  for (const match of stepMatches) {
    ...
    if (m) steps.push(m[2].trim().substring(0, 120));
  }
  ...
  pushEvent('planning' as any, summary, {
    intent,
    approach,
    steps,
    source: 'chat-history-watcher',
...
function handleResponseText(text: string): void {
  ...
  if (statusMatch) {
    ...
    pushEvent('state-change', `${statusVerb}\n${statusDetail}`, {
      state: statusVerb,
      detail: statusDetail,
      source: 'chat-history-watcher',
      autoCapture: true,
...
const TOOL_TYPE_MAP: Record<string, { type: string; extract?: (item: any, msg: string) => Record<string, any> }> = {
...
function handleToolInvocation(item: any): void {
  if (!getActiveSession()) return;
  ...
  if (toolId === 'manage_todo_list' && (toolData.kind === 'todoList' || toolData.todoList)) {
    const todos: Array<{ id: string; title: string; status: string }> = toolData.todoList || [];
    ...
    if (inProgressTodos.length > 0) {
      ...
      pushEvent('state-change', `${statusLabel}\n${current.title}`, {
        state: statusLabel,
      ...
      // All done
      pushEvent('state-change', `Completed\nAll ${todos.length} todos finished`, {
        state: 'Completed',
  ...
  if (!cleanMsg || cleanMsg.length < 5) return;
  ...
  if (mapping) {
    ...
    pushEvent(mapping.type as any, cleanMsg, {
      source: 'chat-history-watcher',
      toolId,
      autoCapture: true,
      ...extraMeta,
...
function trackInputText(text: string): void {
...
export function disposeChatHistoryWatcher(): void {
  disposed = true;
