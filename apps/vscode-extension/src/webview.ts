...
import { getActiveSession, onEventPushed, onSessionChange, type VaiEvent } from './session.js';
import { apiCall } from './api.js';

/* ── Panel State ───────────────────────────────────────────────── */

let panel: vscode.WebviewPanel | null = null;
let allEvents: VaiEvent[] = [];

/* ── Open/Show Panel ───────────────────────────────────────────── */

export function openLogsPanel(context: vscode.ExtensionContext): void {
  if (panel) {
  ...
  const eventSub = onEventPushed((event) => {
    allEvents.push(event);
    sendToPanel({ type: 'newEvent', event });
  });

  const sessionSub = onSessionChange((session) => {
    if (session) {
  ...
  panel.webview.onDidReceiveMessage((msg) => {
    if (msg.type === 'refresh') {
      ...
      if (s) void loadServerEvents(s.id);
