...
import { Bot, Hammer, ListChecks, Swords, ChevronDown } from 'lucide-react';

const MODES: { id: ChatMode; label: string; icon: typeof Bot; shortcut: string }[] = [
  { id: 'agent', label: 'Agent', icon: Bot, shortcut: '1' },
  { id: 'builder', label: 'Builder', icon: Hammer, shortcut: '2' },
  { id: 'plan', label: 'Plan', icon: ListChecks, shortcut: '3' },
  { id: 'debate', label: 'Debate', icon: Swords, shortcut: '4' },
];
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        ...
        title={`Mode: ${current.label} (Ctrl+1-4)`}
      >
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute bottom-full left-0 mb-1 w-44 rounded-lg border border-zinc-700 bg-zinc-900 py-1 shadow-xl">
          {MODES.map((m) => {
            const Icon = m.icon;
