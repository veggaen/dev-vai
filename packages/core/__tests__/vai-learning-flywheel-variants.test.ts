...
// Generated from vai-learning-flywheel.test.ts by scripts/generate-variant-tests.mjs
...
describe('VaiEngine Learning Flywheel', () => {
  ...
  it('learns from high-confidence responses via chat() (variant)', async () => {
  ...
  it('does NOT learn from fallback responses (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
  ...
  it('does NOT learn from conversational greetings (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
    const beforeCount = engine.knowledge.entryCount;
  ...
  it('does NOT learn from math results (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
    const beforeCount = engine.knowledge.entryCount;
  ...
  it('persists auto-learned entries across restarts (variant)', async () => {
  ...
  it('auto-learned entries survive restart via persistence (variant)', async () => {
  ...
  it('deduplicates — does not re-learn already-known topics (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
  ...
  it('does NOT learn when noLearn is true (protective parenting mode) (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
    const beforeCount = engine.knowledge.entryCount;
  ...
  it('does NOT learn when noLearn is true via chatStream() (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
    const beforeCount = engine.knowledge.entryCount;
  ...
  it('invokes the learning hook when noLearn is omitted or false (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
  ...
  it('persists auto-learned entries via the snapshot instead of fuzzy matching (variant)', async () => {
    const engine = new VaiEngine({ persistPath });
