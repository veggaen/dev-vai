...
// Generated from dynamic-chat.test.ts by scripts/generate-variant-tests.mjs
...
describe('dynamic chat behaviour', () => {
  ...
  describe('relative date parsing', () => {
    it('"what day is it in a week?" → Wed May 20 2026 (variant)', async () => {
    ...
    it('"what will the date be in three days?" → Sat May 16 2026 (variant)', async () => {
      const r = await engine.chat({
    ...
    it('"a week from now what day is it?" → Wed May 20 2026 (variant)', async () => {
      const r = await engine.chat({
    ...
    it('"what day was it two weeks ago?" → Wed April 29 2026 (variant)', async () => {
      const r = await engine.chat({
    ...
    it('"what day is the day after tomorrow?" → Fri May 15 2026 (variant)', async () => {
      const r = await engine.chat({
    ...
    it('"not tomorrow but the day after, what day is it?" → Fri May 15 2026 (variant)', async () => {
      const r = await engine.chat({
    ...
    it('"day before yesterday what day was it?" → Mon May 11 2026 (variant)', async () => {
      const r = await engine.chat({
    ...
    it('intro + "not tomorrow but the day after" → greets + Fri May 15 (variant)', async () => {
      const r = await engine.chat({
        messages: [{
          role: 'user',
  ...
  describe('inventor menu for vague follow-ups', () => {
    it('"can you tell me about someone else who invented something else?" → returns a menu (variant)', async () => {
      const r = await engine.chat({
        messages: [
    ...
    it('"who invented something else?" still returns the menu (variant)', async () => {
      const r = await engine.chat({
    ...
    it('does NOT trigger the menu for a specific topic like "who invented the telephone?" (variant)', async () => {
      const r = await engine.chat({
  ...
  describe('self-reflection / meta questions about Vai', () => {
    it('"what do you think is your own best upgrade to make yourself better?" → candid honest answer (variant)', async () => {
      const r = await engine.chat({
        messages: [
          {
            role: 'user',
    ...
    it('"what would you improve about yourself?" hits self-reflection (variant)', async () => {
      const r = await engine.chat({
    ...
    it('"what are your weaknesses?" hits self-reflection (variant)', async () => {
      const r = await engine.chat({
    ...
    it('does NOT trigger self-reflection for unrelated improvement questions (variant)', async () => {
      const r = await engine.chat({
  ...
  describe('combined intro + new date phrasings', () => {
    it('"Hi I am Peter, what day is it in a week and who invented the airplane?" composes correctly (variant)', async () => {
      const r = await engine.chat({
        messages: [
          {
            role: 'user',
