...
// Generated from edge-cases.test.ts by scripts/generate-variant-tests.mjs
...
describe('VaiEngine edge cases', () => {
  ...
  it('returns a non-empty string for an empty messages array (variant)', async () => {
  ...
  it('handles empty user content without throwing (variant)', async () => {
  ...
  it('handles whitespace-only content (variant)', async () => {
    const response = await engine.chat({ messages: [{ role: 'user', content: '    \t\n\r  ' }] });
    expect(typeof response.message.content).toBe('string');
    expect(response.message.content.length).toBeGreaterThan(0);
  });

  it('accepts null content (non-string) without crashing (variant)', async () => {
  ...
  it('accepts undefined content without crashing (variant)', async () => {
    const response = await engine.chat({
  ...
  it('accepts numeric content without crashing (variant)', async () => {
    const response = await engine.chat({
  ...
  it('accepts object content without crashing (variant)', async () => {
    const response = await engine.chat({
  ...
  it('handles unicode zero-width characters (variant)', async () => {
    const response = await engine.chat({
  ...
  it('handles RTL override and bidi control characters (variant)', async () => {
    const response = await engine.chat({
  ...
  it('handles lone surrogate halves without throwing (variant)', async () => {
    const response = await engine.chat({
  ...
  it('handles very long input in under 5 seconds (variant)', async () => {
  ...
  it('resists regex backtracking on adversarial input (variant)', async () => {
  ...
  it('does not echo a prompt-injection payload verbatim as its own instruction (variant)', async () => {
  ...
  it('strips HTML/script-like content from the output path (variant)', async () => {
  ...
  it('chatStream does not throw on empty messages array (variant)', async () => {
    const chunks: string[] = [];
  ...
  it('chatStream handles null content (variant)', async () => {
    const chunks: string[] = [];
