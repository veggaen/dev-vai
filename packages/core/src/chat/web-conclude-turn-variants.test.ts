...
// Generated from web-conclude-turn.test.ts by scripts/generate-variant-tests.mjs
...
describe('web-conclude-turn', () => {
  it('detects substantive questions for web conclusion (variant)', () => {
  ...
  it('expands short follow-ups with prior user context (variant)', () => {
  ...
  it('expands decorated conversational cues instead of searching filler words (variant)', () => {
  ...
  it('walks past terse formatting turns when expanding a later contextual cue (variant)', () => {
  ...
  it('returns synthesized text when search returns sources (variant)', async () => {
  ...
  it('skips when testMode is on (variant)', async () => {
  ...
  it('skips web search when the system prompt requires learned browsing memory only (variant)', async () => {
    const search = vi.fn();
    const result = await tryWebConcludeTurn(
  ...
  it('detects explicit source-reference requests without confusing source code with citations (variant)', () => {
  ...
  it('builds a strict citation contract for retrieved web evidence (variant)', () => {
  ...
  it('keeps source-backed casual answers from turning into research reports (variant)', () => {
    const prompt = buildEvidenceContextSystemHint(
  ...
  it('gives stable how-to questions to local routes before attempting web search (variant)', async () => {
    const search = vi.fn();
    const result = await tryWebConcludeTurn(
  ...
  it('bypasses local defer when a stable explanation explicitly asks for sources (variant)', async () => {
  ...
  it('keeps source-code examples on the local-first path (variant)', async () => {
    const search = vi.fn();
    const result = await tryWebConcludeTurn(
  ...
  it('gives contextual follow-ups to local routes before attempting web search (variant)', async () => {
    const search = vi.fn();
    const result = await tryWebConcludeTurn(
  ...
  it('does not search context-free rewrite fragments (variant)', async () => {
    const search = vi.fn();
    const result = await tryWebConcludeTurn(
  ...
  it('gives local architecture planning routes the first chance to answer (variant)', async () => {
    const search = vi.fn();
    const result = await tryWebConcludeTurn(
  ...
  it('still searches recency-sensitive how-to questions (variant)', async () => {
  ...
  it('fetchTurnWebEvidence can still search stable list prompts when ignoreLocalDefer is set (variant)', async () => {
    const search = vi.fn(async () => ({
  ...
  it('fetchTurnWebEvidence searches stable prompts when sources are explicitly requested (variant)', async () => {
