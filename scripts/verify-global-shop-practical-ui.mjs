...
const PROMPT = process.env.VAI_GLOBAL_UI_PROMPT || '¿A qué hora abre Zara Gran Vía Madrid?';
const OUTPUT_DIR = path.resolve('screenshots/global-shop-practical');
...
bootstrap.auth = {
  ...
  enabled: false,
  authenticated: true,
  user: { id: 'global-shop-visual', email: 'global-shop@test.local', name: 'Global Shop QA' },
};
...
try {
  ...
  await page.waitForFunction(
    ({ selector, previous }) => {
      ...
      return /Opening hours/i.test(latest) && /10:00\s*[-–]\s*22:00/.test(latest);
    },
  ...
  const answerScreenshot = path.join(OUTPUT_DIR, 'zara-madrid-answer.png');
  ...
  const sourceScreenshot = path.join(OUTPUT_DIR, 'zara-madrid-source-open.png');
  await page.screenshot({ path: sourceScreenshot, fullPage: true });

  const failures = [];
  if (!/10:00\s*[-–]\s*22:00/.test(answerText)) failures.push('verified store hours missing');
  if (/10:30/.test(answerText)) failures.push('model-mutated opening time leaked into the answer');
  ...
  if (!/zara\.com/i.test(sourceText) || !/ZARA MADRID GRAN VIA/i.test(sourceText)) failures.push('official Zara branch source is not visible');
