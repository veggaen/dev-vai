 ...
 *   node scripts/conv-loop.mjs --draft-trace    # log self-eval drafts (incl. cap-suppressed) to artifacts/corpus-runs/draft-trace-<ts>.jsonl
 *
 * Exits 0 if all turns pass, 1 otherwise.
 */

import { performance } from 'node:perf_hooks';
import { existsSync, watch, mkdirSync, appendFileSync } from 'node:fs';
...
const draftTrace = args.includes('--draft-trace');

// Draft-trace setup (Addition #2 from docs/capabilities/self-evaluation.md).
// One JSONL file per run, all turns appended. Cap-suppressed diagnostic third drafts
// are generated by the engine sink and logged with capSuppressed=true; never emitted.
const DRAFT_TRACE_DIR = join(process.cwd(), 'artifacts', 'corpus-runs');
const DRAFT_TRACE_RUN_ID = `${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}`;
const DRAFT_TRACE_PATH = join(DRAFT_TRACE_DIR, `draft-trace-${DRAFT_TRACE_RUN_ID}.jsonl`);
if (draftTrace) {
  mkdirSync(DRAFT_TRACE_DIR, { recursive: true });
}
function draftTraceWrite(record) {
  if (!draftTrace) return;
  appendFileSync(DRAFT_TRACE_PATH, JSON.stringify(record) + '\n');
}
...
async function runConversation(spec) {
  ...
  // Wire the engine's self-eval draft-trace sink to our JSONL writer.
  // We tag each record with conversation + turn ordinal + spec id for joinability.
  let __turnOrdinal = -1;
  if (draftTrace && typeof engine.setDraftTraceSink === 'function') {
    engine.setDraftTraceSink(({ input, verdict, capSuppressed }) => {
      draftTraceWrite({
        ts: new Date().toISOString(),
        runId: DRAFT_TRACE_RUN_ID,
        specId: spec.id,
        turnOrdinal: __turnOrdinal,
        input,
        verdict: {
          kind: verdict.kind,
          revisionApplied: verdict.revisionApplied,
          capFired: verdict.capFired,
          failedPredicates: verdict.failedPredicates,
          lastHint: verdict.lastHint,
        },
        trace: verdict.trace,
        capSuppressed: capSuppressed ?? null,
      });
    });
  }
  const messages = [];
